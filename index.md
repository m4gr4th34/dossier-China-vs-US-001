...
This dossier is a two-country **achievement ledger**: a per-row-verified record of American and Chinese innovation, infrastructure, and social achievements from 1926 to 2026, built under a published selection rule and a published promotion standard. It is deliberately neither a triumphalist nor a declinist account — every row wears an honest status label, and the momentum layer is computed only as an explicitly **OPEN-CAVEATED** constructed index — shown with its full sensitivity bands and named exclusions, never as a measured verdict. The honest state today: 294 candidate rows, 290 ESTABLISHED, the rest one open challenge (OPEN-UNVERIFIED) and three provenance-flagged REPORTED items. **Don't trust this paper — run it.**

## Avenues

| Avenue | Thesis | Status | Forecast | Sources |
|---|---|---|---|---|
| The achievement ledger | A 294-row, event-anchored, per-row-verified corpus of US and Chinese achievements 1926–2026; 290 rows ESTABLISHED under a published promotion standard. The full 1926–2025 chronological corpus is processed, plus author-proposed per-row passes (batch 11: physical-manufacturing and 2023–2025 achievements; batch 12: maritime and defense-industrial; batch 13: the national-security view) adding rows on both sides — each through the same ritual. Rows also carry a country-blind natsec / dual-use tag (Amendment 5) driving Figure III. | ESTABLISHED | — | verification_ritual.md, selection_criteria.md |
...
- [PASS] Census: 290 ESTABLISHED / 1 OPEN-UNVERIFIED / 3 REPORTED (live corpus counts)
...
The ledger itself — 294 rows, event-anchored, per-row-verified, 290 ESTABLISHED under the promotion standard — is the dossier's spine and its most finished part. It is asserted directly, because documented, dated, sourced achievements are bedrock, and timidity about them would be its own dishonesty.
